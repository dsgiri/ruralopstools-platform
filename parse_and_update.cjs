...
// Update docs/tools/*.md files
const toolsDir = 'docs/tools';
if (fs.existsSync(toolsDir)) {
    const files = fs.readdirSync(toolsDir);
    files.forEach(file => {
        if (file.endsWith('.md')) {
            const toolName = file.replace('.md', '').toLowerCase();
            if (toolData[toolName]) {
                const filePath = path.join(toolsDir, file);
                let content = fs.readFileSync(filePath, 'utf8');
                
                // Replace the paragraph after # ToolName
                // Assuming format:
                // # ToolName
                // <long desc>
                // ## Links
                const newDesc = toolData[toolName].desc;
                // find where ## Links starts
                const linksIdx = content.indexOf('## Links');
                let newContent = '';
                if (linksIdx !== -1) {
                    const headerPart = content.substring(0, content.indexOf('\n') + 1);
                    const linksPart = content.substring(linksIdx);
                    newContent = headerPart + '\n' + newDesc + '\n\n' + linksPart;
                } else {
                    // Try to just replace after heading
    ...
    console.log('Updated docs/tools/*.md');
}

// Update src/data.ts
let dataRaw = fs.readFileSync('src/data.ts', 'utf8');
let updatedData = dataRaw.replace(/\{\s*id:\s*'([^']+)',\s*name:\s*'([^']+)',\s*description:\s*'([^']*)',\s*icon:\s*([^,]+),\s*color:\s*'([^']+)',\s*bg:\s*'([^']+)',\s*url:\s*'([^']+)',\s*github:\s*'([^']+)'\s*(?:,\s*longDescription:\s*'([^']*)'\s*)?\}/g, (match, id, name, oldDesc, icon, color, bg, url, github, oldLongDesc) => {
    const key = name.toLowerCase();
    
    // Default to existing or something if missing
    let finalDesc = oldDesc;
    let finalLongDesc = oldLongDesc || oldDesc; 
    let finalUrl = url;

    if (toolData[key]) {
        finalDesc = toolData[key].desc;
        finalLongDesc = finalDesc; // using the same simple description as per request? The user said "update all tools md files with Short-Desc". Then they said "Also we need simple easy to read and understand language description" which is the same as the short desc. We'll set description to the new one, and we can remove longDescription or set it to the new one.
        finalUrl = `https://${toolData[key].url}`;
    }
    
    // Fix Predictor url (it has predictor.predictor.ruralopstools.com in the prompt by mistake probably, let's fix it)
    if (key === 'predictor') {
        finalUrl = 'https://predictor.ruralopstools.com';
    }

    const safeDesc = finalDesc.replace(/'/g, "\\'");
    let newStr = `{ id: '${id}', name: '${name}', description: '${safeDesc}', icon: ${icon}, color: '${color}', bg: '${bg}', url: '${finalUrl}', github: '${github}'`;
    // We'll keep longDescription the same as description or remove it? Let's just remove longDescription since they want it simple.
    // Or we keep longDescription as the safeDesc so the tool details page shows it cleanly without being cut off.
    newStr += `, longDescription: '${safeDesc}' }`;
    return newStr;
});
